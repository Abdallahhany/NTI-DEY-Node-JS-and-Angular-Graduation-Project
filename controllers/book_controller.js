...
class Book{
    static allBooks = async (req,res)=>{
        try{
            ...
            res.status(500).send({success:true,err:e.message,data:e});
        }
    }
    ...
    static addBook = async (req,res)=>{
        try{
            ...
            if(req.file) fs.unlinkSync(path.join(__dirname,'../images/'+req.file.filename));
            res.status(500).send({success:false,err:e.message,data:e});
        }
    }
    static updateBook = async (req,res)=>{
        ...
        try{
            ...
            await book.save();
            if(imageChanged) fs.unlinkSync(path.join(__dirname,'../images/'+prevImg));
            res.status(200).send({success:true,data:book});
        }catch(e){
            res.status(500).send({success:false,err:e.message,data:e});
        }
